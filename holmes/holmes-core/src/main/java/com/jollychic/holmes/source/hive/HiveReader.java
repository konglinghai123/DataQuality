 ...
 */
@Slf4j
public class HiveReader extends SourceReaderDefault {
    public static Map<String, HiveConnection> conns = Maps.newHashMap();
    private HiveConnection hiveConnection = null;

    @Override
    public void init(SourceConfig sourceConfig) {
        super.init(sourceConfig);
        HiveConfig hiveConfig = (HiveConfig) sourceConfig.getConnConfig();
        String url = "jdbc:hive://"+hiveConfig.getHost()+":"+hiveConfig.getPort()+"/"+hiveConfig.getDatabase();
        String user = hiveConfig.getUser();
        String passwd = AESUtils.AES_CBC_Decrypt(hiveConfig.getPasswd(), AESUtils.DEFAULT_PASSWORD);
        synchronized (this) {
            if (conns.get(url+user+passwd) != null) {
                hiveConnection = conns.get(url+user+passwd);
            } else {
                try {
                    hiveConnection = new HiveConnection(url, user, passwd);
                } catch (SQLException e) {
                    throw new ServiceException(ErrorCode.SOURCE_CONNECTION_ERROR, "hive connection error");
                }
                conns.put(url+user+passwd, hiveConnection);
            }
        }
    }
    ...
    public List<String> read() {
        ...
        try {
            ResultSet resultSet = hiveConnection.executeQuery("SELECT * FROM "+sourceConfig.getTableName()+" WHERE "+
    ...
    public List<String> read(String sql) {
        List<String> result = Lists.newArrayList();
        try {
            ResultSet resultSet = hiveConnection.executeQuery(sql);
            ResultSetMetaData md = resultSet.getMetaData();
            while (resultSet.next()) {
                Map<String, Object> row = Maps.newHashMap();
                for(int i=1; i<=md.getColumnCount(); i++) {
    ...
    public long getCount() {
        long result = 0;
        try {
            ResultSet resultSet = hiveConnection.executeQuery("SELECT COUNT(*) FROM "+sourceConfig.getTableName()+" WHERE "+
                    sourceConfig.getPartitionName()+"='"+sourceConfig.getPartitionValue()+"';");
            if(resultSet.next()) {
    ...
    public List<String> getTables() {
        List<String> result = Lists.newArrayList();
        try {
            ResultSet resultSet = hiveConnection.executeQuery("SHOW tables;");
            while (resultSet.next()) {
    ...
    public void validateConnectionInfo(ConnConfig connConfig) {
        HiveConfig hiveConfig = (HiveConfig) connConfig;
        String url = "jdbc:hive://"+hiveConfig.getHost()+":"+hiveConfig.getPort()+"/"+hiveConfig.getDatabase();
        String user = hiveConfig.getUser();
        String passwd = AESUtils.AES_CBC_Decrypt(hiveConfig.getPasswd(), AESUtils.DEFAULT_PASSWORD);
    ...
    public String getTableParam(String connectionInfo, String tableName) {
        ...
        sourceConfig.setConnConfig(JSON.parseObject(connectionInfo, HiveConfig.class));
        sourceConfig.setTableName(tableName);
        init(sourceConfig);
        List<String> columns = Lists.newArrayList();
        try {
            ResultSet resultSet = hiveConnection.executeQuery("show columns from "+sourceConfig.getTableName());
            while (resultSet.next()) {
